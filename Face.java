...
public class Face extends OvalDraw {
	...
	public void paintComponent(Graphics g) {
		...
		int min = 1;
		int max = 3;
		int smileType =(int)(Math.random() * (max - min + 1) + min);

		if (smileType == 1) {
			g.drawArc(getPositionX() + 5, getPositionY(), getWidth() - 10, getHeight() - 10, -45, -90); // draws smile
		}
		else if (smileType == 2) {
			g.drawArc(getPositionX() + 5, getPositionY() + (getHeight() / 2), getWidth() - 10, getHeight() - 10, -45, -90); // draws frown
		}
		else if (smileType == 3) {
			g.drawLine(getPositionX() + (getWidth()/4), getPositionY() + (getHeight()/2), getWidth(), getPositionY()+(getHeight()/2)); // draws neutral
		}
	}
}